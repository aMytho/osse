...
use crate::{
    api::{
        albums::album_service::AlbumService, artists::artist_service::ArtistService,
        tracks::track_service::TrackService,
    },
    entities::track::Track,
    files::get_file_directory,
};

use self::{
    formats::tag_extractor::TagExtractor,
    metadata::{CoverArt, TagMetadata},
};
...
pub async fn scan_files(
    ...
    for file in files {
        ...
        meta.disc_number = tag_meta.disc_number;

        meta.year = tag_meta.year.map(|f| f.try_into().unwrap_or_default());
        meta.track_number = tag_meta
            .track_index
            .map(|f| f.try_into().unwrap_or_default());
        ...
        if tag.picture_count() > 0 {
            return Some(CoverArt {
                ...
                mime_type: util::get_mime_type(
                    tag.pictures()[0]
                        .mime_type()
                        .unwrap_or(&lofty::picture::MimeType::Unknown(String::new())),
                ),
            });
        }
    }
